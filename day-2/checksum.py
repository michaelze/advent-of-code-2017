    ...
    def compute(input):
        ...
        for row in range(0, len(input)):
            ...
            for column in range(0, len(input[row])):
                ...
                if smallest == None or smallest > current:
                    smallest = current
                if largest == None or largest < current:
                    largest = current
            checksum = checksum + (largest - smallest)
        return checksum

    def computeDividing(input):
        checksum = 0
        for row in range(0, len(input)):
            sortedRow = sorted(input[row], reverse = True)
            for column in range(0, len(sortedRow) - 1):
                for divisorColumn in range(column + 1, len(sortedRow)):
                    divisor = sortedRow[divisorColumn]
                    if sortedRow[column] % divisor == 0:
                        checksum = checksum + int(sortedRow[column] / divisor)
        return checksum
...
class ChecksumTest(unittest.TestCase):
    ...
    def testCompute(self):
        ...
        self.assertEqual(Checksum.compute(grid), 18)

    def testComputeDividing(self):
        grid = [[5, 9, 2, 8], [9, 4, 7, 3], [3, 8, 6, 5]]
        self.assertEqual(Checksum.computeDividing(grid), 9)
...
if __name__ == '__main__':
    ...
    print ("checksum: " + str(Checksum.compute(input)))
    print ("checksum dividing: " + str(Checksum.computeDividing(input)))
